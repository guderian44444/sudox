...
1. 完成舊 session 時，舊程式用 `current + 1` 推進下一層。若排行榜／雲端已把下一層抬到 29，再完成第 27 層會錯跳 30。v51 改為 `max(既有下一層, 完成層 + 1)`。
2. 阿霖模式沿用所選正式難度的進度欄，會讓阿霖過關污染高手等正式層數。v51 新增獨立 `floors.alin`；舊存檔會依玩家自己的阿霖排行榜做一次性遷移。
