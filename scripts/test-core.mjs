...
import { exportSaveCode, importSaveCode, mergeProgressHighWater, nextFloorFromCompleted, parseSaveCode, preferSaveSide, raiseFloorProgress, reconcileFloorsFromLeaderboardRows, rewardProgress, saveProgress as writeProgress, saveTimestampMs, sessionFloorBehindProgress } from "../src/state/store.js";
...
assert(/placement === "board-edge"/.test(appSource) && /querySelector\("\.sudoku-board"\)[\s\S]*?getBoundingClientRect/.test(appSource), "completed unit effects should anchor to the Sudoku board geometry");
assert(/boardRect\.top < 104/.test(appSource) && /window\.scrollBy\(\{ top: boardRect\.top - 104, behavior: "instant" \}\)/.test(appSource), "board-edge celebrations should restore the board edge when keypad focus scrolls it offscreen");
assert(/event\.type}-\$\{variant}`, "board-edge"/.test(appSource), "row, column, and box celebrations should request board-edge placement");
assert(/\.game-effect\.board-edge \{[^}]*top:\s*var\(--board-edge-y[^}]*transform:\s*translate\(-50%, -100%\)/.test(stylesheet) && /@keyframes board-edge-effect-life/.test(stylesheet), "board-edge celebrations should stay completely above the Sudoku grid");
...
assert(/const cloud = parseSaveCode\(saveCode\)/.test(appSource) && /const imported = \{ \.\.\.cloud, session: null \}/.test(appSource), "cloud hydration should keep cloud progress timestamp while ignoring active sessions");
...
const persistSessionBlock = appSource.slice(appSource.indexOf("function persistSession"), appSource.indexOf("function islandOwner"));
assert(/cloudProgressSaveCode\(progress\)/.test(appSource) && /exportSaveCode\(nextProgress, null\)/.test(appSource), "雲端存檔應只保存永久進度，不可攜帶數獨中途盤面");
assert(/cloudHasSession: false/.test(appSource) && /function persistSession[\s\S]*saveSession\(session\)[\s\S]*clearSession\(\)/.test(appSource) && !/scheduleCloudSync\(\)/.test(persistSessionBlock), "數獨遊戲過程應只寫本機 session，不可觸發小島雲端同步");
...
const leaderboardCaughtUp = reconcileFloorsFromLeaderboardRows(
  { floors: { easy: 31, medium: 2, hard: 3, alin: 1 } },
  [
    { difficulty: "easy", floor: 31 },
    { difficulty: "medium", floor: 3 },
    { difficulty: "hard", floor: 3 },
    { difficulty: "alin", floor: 4 }
  ]
);
assert(leaderboardCaughtUp.floors.easy === 32 && leaderboardCaughtUp.floors.medium === 4 && leaderboardCaughtUp.floors.hard === 4 && leaderboardCaughtUp.floors.alin === 5, "排行榜已完成層數應把各難度下一關只往前校正");
...
assert(/reconcileLeaderboardFloorProgress/.test(appSource) && /fetchPlayerLeaderboardRows/.test(appSource), "各難度下一關應從排行榜高水位校正");
...
const loggedScore = buildScore({ ...imported.progress, floors: { ...imported.progress.floors, hard: 19, alin: 18 } }, { difficulty: "hard", floor: 18, stars: 1, elapsed: 830, mistakes: 3 }, false, { appVersion: "v57" });
assert(loggedScore.p_floor === 18 && loggedScore.p_next_floor === 19 && loggedScore.p_app_version === "v57", "排行榜 LOG 應帶完成層、當時下一層與 app 版次");
