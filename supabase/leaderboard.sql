...
begin
  if char_length(trim(p_player_name)) not between 1 and 16
    ...
    or p_mistakes < 0
    or p_stars not between 1 and 3 then
    raise exception 'Invalid leaderboard score';
  end if;

  insert into public.leaderboard_scores (
    player_id, player_name, difficulty, floor, score, elapsed_seconds, mistakes, stars
  ) values (
    p_player_id, trim(p_player_name), p_difficulty, p_floor, p_score, p_elapsed_seconds, p_mistakes, p_stars
  )
